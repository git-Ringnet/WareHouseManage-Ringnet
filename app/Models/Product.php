...
class Product extends Model
{
    ...
    public function getAllProduct($filters = [], $perPage, $status = [], $products_name = null, $sn = null, $providearr, $unitarr, $taxarr, $keywords = null, $sortByArr = null)
    {
        ...
        $products = DB::table($this->table)
            ...
            ->addSelect(DB::raw("(SELECT GROUP_CONCAT(serinumbers.serinumber) FROM 
            serinumbers WHERE seri_status != 3 AND serinumbers.product_id = product.id) as serinumbers"));
        ...
        $products = $products->orderBy('product.id', 'asc')->paginate($perPage);

        return $products;
    }
    public function serinumbers()
    {
        return $this->hasMany(Serinumbers::class, 'product_id', 'id');
    }
