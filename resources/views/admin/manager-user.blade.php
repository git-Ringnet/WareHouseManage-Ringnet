            ...
            <div class="row m-auto filter pt-2">
                <form class="w-100" action="" method="get" id='search-filter'>
                    ...
                    <div class="d-flex justify-contents-center align-items-center mr-auto row-filter my-3 m-0">
                        ...
                        if ($fullUrl === route('admin.manageruser')) {
                                ...
                                <div class="filter-options">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" id="dropdown-menu"
                                            ...
                                            <div class="scrollbar">
                                                <button class="dropdown-item" id="btn-name">Tên người dùng</button>
                                                <button class="dropdown-item d-none" id="btn-roles">Vai trò</button>
                                        ...
                                        @if (!empty($string))
                                            <a class="btn-delete-filter"
                                                href="{{ route('admin.manageruser') }}"><span>Tắt bộ lọc</span></a>
                                        @endif
                                    </div>
                                    <?php $status = [];
                                    $roles = [];
                                    ...
                                    <tr>
                                            ...
                                            <span class="d-flex align-items-center">
                                                <a href="#" class="sort-link" data-sort-by="id"
                                                    data-sort-type="{{ $sortType }}"><button class="btn-sort"
                                                        type="submit">Mã người dùng</button></a>
                                        ...
                                        <th>
                                            <span class="d-flex align-items-center">
                                                <a href="#" class="sort-link" data-sort-by="name"
                                                    data-sort-type="{{ $sortType }}"><button class="btn-sort"
                                                        type="submit">Họ tên người dùng</button></a>
                                        ...
                                        <th>
                                            <span class="d-flex justify-content-center">
                                                <a href="#" class="sort-link" data-sort-by=""
                                                    data-sort-type="{{ $sortType }}"><button class="btn-sort"
                                                        type="submit">Ngày còn lại</button></a>
                                                <div class="icon" id="icon"></div>
                                            </span>
                                        </th>
                                        ...
                                        <th>
                                            <span class="d-flex justify-content-center">
                                                <a href="#" class="sort-link" data-sort-by=""
                                                    data-sort-type="{{ $sortType }}"><button class="btn-sort"
                                                        type="submit">Hoạt động lần cuối</button></a>
                                                <div class="icon" id="icon"></div>
                                            </span>
                                        </th>
                                        </form>
                                ...
                                <tbody>
                                    @foreach ($usersList as $value)
                                        <tr>
                                            ...
                                            <td class="d-none">{{ $value->role_name }}</td>
                                            <td>{{ $value->phonenumber }}</td>
                                            <td>{{ $value->email }}</td>
                                            {{-- <td class="text-center">{!! $value->status == 0
                                                ? '<button type="submit" class="btn btn-sm btn-success">Active</button>'
                                                : ' <button type="submit" class="btn btn-sm btn-primary">Disable</button>' !!}</td> 
                                            ...
                                            <td class="text-center">
                                                <select class="p-1 px-2 status-select"
                                                    ...
                                                    @foreach ($licenses as $item)
                                                        <option id="{{ $value->id }}"
                                                            value="{{ $item->id }}"<?php if ($value->namelicense == $item->license_name) {
                                                                echo 'selected';
                                                            } ?>>
                                                            {{ $item->license_name }}
                                                        </option>
                                            ...
                                            <td class="text-center">
                                                @php
                                                    $endDate = \Carbon\Carbon::parse($value->date_end);
                                                    $daysDifference = $endDate->diffInDays(\Carbon\Carbon::now());
...
<script>
    ...
    $(document).ready(function() {
        $('.status-select').change(function() {
            var newLicense = $(this).val();
            var idUser = $(this).find('option:selected').attr(
                'id');
            console.log("Val: " + newLicense, "ID: " + idUser);
            $.ajax({
                url: '{{ route('updateLicense') }}',
                type: 'GET',
                data: {
