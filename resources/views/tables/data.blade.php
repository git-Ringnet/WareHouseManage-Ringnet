              ...
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <th><a href="{{route('data.show',$value->id)}}">{{$value->products_code}}</a></th>
                    <th>{{$value->products_name}}</th>
                    <th>
                      <select class="product_category" name="product_category" id="{{$value->id}}">
                        @foreach($category as $va)
                        <option value="{{$va->id}}" {{($va->id == $value->getCategory->id) ? 'selected' : '' }}>{{$va->category_name}}</option>
                        @endforeach
                      </select>
                    </th>
                    <th>{{$value->products_trademark}}</th>
                    <th>Tồn kho</th>
                    <th>Trị trung bình</th>
                    <th>Trị tồn kho</th>
                    <th>Trạng thái</th>
                    <th>
                      <div class="icon">
