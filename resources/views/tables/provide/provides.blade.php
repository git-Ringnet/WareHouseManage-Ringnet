            ...
            <div class="row m-auto filter pt-2">
                <form class="w-100" action="" method="get" id='search-filter'>
                    ...
                    <div class="d-flex justify-contents-center align-items-center mr-auto row-filter my-3 m-0">
                        ...
                        if ($fullUrl === route('provides.index')) {
                ...
                <div class="row action">
                    ...
                    {{-- <div class="dropdown my-2 ml-4">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            ...
                            <button id="disableStatusProvide" class="dropdown-item">Disable</button>
                        </div>
                    </div> --}}
                </div>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    {{-- <div class="paginator mt-4 d-flex justify-content-end">
                        {{ $provides->appends(request()->except('page'))->links() }}
                    </div> --}}
                    @if ($provides->count() > 0)
                        @php
                            $paginationRange = App\Helpers\PaginationHelper::calculatePaginationRange($provides->currentPage(), $provides->lastPage());
                        @endphp
                        <div class="pagination mt-4 d-flex justify-content-end">
                            <ul>
                                @if ($paginationRange['start'] > 1)
                                    <li><a href="{{ $provides->url(1) }}">1</a></li>
                                ...
                                @for ($i = $paginationRange['start']; $i <= $paginationRange['end']; $i++)
                                    <li class="{{ $i == $provides->currentPage() ? 'active' : '' }}">
                                        <a href="{{ $provides->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor

                                @if ($paginationRange['end'] < $provides->lastPage())
                                    @if ($paginationRange['end'] < $provides->lastPage() - 1)
                                        <li><span>...</span></li>
                                    @endif
                                    <li><a
                                            href="{{ $provides->url($provides->lastPage()) }}">{{ $provides->lastPage() }}</a>
                                    </li>
                                @endif
                            </ul>
                        </div>
...
<script>
    ...
    // AJAX disable user
    $(document).on('click', '#disableStatusProvide', function(e) {
        e.preventDefault();
        if (myFunctionCancel()) {
            const list_id = [];
            ...
            $.ajax({
                url: "{{ route('disableStatusProvide') }}",
                type: "get",
                data: {
                    list_id: list_id,
                },
    ...
    // AJAX disable user
    $(document).on('click', '#activeStatusProvide', function(e) {
            e.preventDefault();
            if (myFunctionCancel()) {
                const list_id = [];
                $('input[name="ids[]"]').each(function() {
    ...
    // AJAX Xóa Exports
    $(document).on('click', '#deleteListProvides', function(e) {
        e.preventDefault();
        if (myFunction()) {
            const list_id = [];
            $('input[name="ids[]"]').each(function() {
